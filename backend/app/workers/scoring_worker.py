...
async def process_scoring(raw_payload: str) -> None:
    job = parse_job(QUEUE_KEY, raw_payload)
    if job.account_id is None:
        log.error("scoring.invalid_payload", payload=raw_payload)
        return
...
async def run_scoring_worker() -> None:
    ...
    while True:
        try:
            ...
            if item:
                _, raw_payload = item
                await process_scoring(raw_payload)
        except Exception as e:
