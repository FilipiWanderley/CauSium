...
export function KpiCard({
  ...
  return (
    <div
      className={clsx(
        'rounded-panel border border-slate-200 bg-white shadow-panel transition-shadow hover:shadow-panel-hover',
        'border-l-[3px]',
        toneStyles[tone],
        compact ? 'p-4' : 'p-5',
      )}
    >
      <div className="flex items-start justify-between gap-3">
        <div className="flex-1 min-w-0">
          ...
          <p
            className={clsx(
              'mt-1.5 font-bold tracking-tight tabular-nums text-slate-900',
          ...
          {delta !== undefined && (
            <div className="mt-2 flex items-center gap-1.5">
              ...
              {deltaLabel && (
                <span className="text-xs text-slate-400 ml-0.5">{deltaLabel}</span>
              )}
            </div>
          )}
        </div>
