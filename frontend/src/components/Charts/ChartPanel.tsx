...
function ChartSkeleton({ height }: { height: number | string }) {
  return (
    <div
      className="animate-pulse rounded-lg bg-gray-light/50"
    ...
    >
      <div className="flex h-full items-end justify-between gap-1 px-6 pb-4 pt-8">
        {Array.from({ length: 12 }).map((_, i) => (
          <div
            key={i}
            className="flex-1 rounded-t bg-gray-light/70"
...
function EmptyChart({ message, height }: { message: string; height: number | string }) {
  return (
    <div
      className="flex items-center justify-center rounded-lg border-2 border-dashed border-gray-light bg-gradient-to-br from-white to-gray-light/30"
      style={{ height: typeof height === 'number' ? `${height}px` : height }}
    >
      <div className="text-center">
        <div className="mx-auto mb-3 flex h-14 w-14 items-center justify-center rounded-xl bg-white shadow-card-premium">
...
export function ChartPanel({
  ...
  return (
    ...
    >
      {/* Header */}
      <div className="flex items-start justify-between gap-3 border-b border-gray-light/50 px-5 py-4">
        <div className="flex items-start gap-3 min-w-0 flex-1">
          {icon && (
            <div className="flex h-9 w-9 shrink-0 items-center justify-center rounded-lg bg-gray-light/50 text-navy">
              {icon}
            </div>
          )}
