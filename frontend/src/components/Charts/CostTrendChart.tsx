...
function CustomTooltip({ active, payload }: any) {
  ...
  return (
    <div className="w-max max-w-[220px] rounded-xl border border-gray-light bg-white p-3 shadow-panel-elevated">
      <div className="flex items-start justify-between gap-2">
        <div>
          <p className="text-[10px] font-semibold uppercase tracking-[0.12em] text-teal-600">Cost</p>
          <p className="mt-0.5 text-xs font-semibold text-navy">{point?.dateFullLabel ?? point?.dateLabel}</p>
        </div>
        {events.length > 0 && (
      ...
      <div className="mt-2 rounded-lg bg-gradient-to-r from-teal-50 to-teal-100/50 px-2.5 py-2">
        <div className="text-[10px] font-medium uppercase tracking-wide text-teal-600/70">Daily cost</div>
        <div className="mt-0.5 text-lg font-bold text-navy">{formatMoney(cost, currency)}</div>
      </div>

      <div className="mt-2 grid grid-cols-2 gap-1.5 text-[11px]">
        <div className="rounded-lg border border-gray-light bg-gray-light/30 px-2 py-1.5">
          <div className="text-gray-cool">Prior</div>
        ...
        <div className="rounded-lg border border-gray-light bg-gray-light/30 px-2 py-1.5">
          ...
          <div className={delta == null ? 'mt-0.5 font-semibold text-slate-700' : delta >= 0 ? 'mt-0.5 font-semibold text-rose-600' : 'mt-0.5 font-semibold text-teal-600'}>
            {delta == null
              ? '—'
      ...
      {events.length > 0 && (
        <div className="mt-2 border-t border-gray-light pt-2 space-y-1.5">
          <p className="text-[10px] font-semibold uppercase tracking-[0.12em] text-gray-cool">
            Events
          </p>
          {events.map((ev) => (
            <div key={ev.id} className="flex items-start gap-1.5">
              ...
              />
              <div>
                <span style={{ color: EVENT_COLOR[ev.event_type] }} className="text-[10px] font-semibold">
                  {EVENT_LABEL[ev.event_type]}
                </span>
                <p className="text-[11px] leading-tight text-slate-600">{ev.title}</p>
